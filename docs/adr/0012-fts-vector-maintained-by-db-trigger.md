...
Decision: a `BEFORE INSERT OR UPDATE` trigger (created via `DB::statement()` in the migration) resolves the text-search config from `language` (fallback `simple`) and builds the weighted vector. PHP treats the column as read-only: `Document::$hidden = ['search_vector']`, never written, only read via FTS.

Trade-offs:
- Invisible from the model — a dev reading only the PHP won't know the column is auto-maintained. This ADR + the `$hidden` line are the signposts.
